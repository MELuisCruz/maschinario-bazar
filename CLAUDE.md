...
> Documento que gobierna **cómo Claude Code construye, versiona, prueba y publica** la app. Consume el resto del diseño, que vive en `docs/`: `docs/PRD.md`, `docs/ARCHITECTURE.md`, `docs/DATA_MODEL.md`, `docs/INTEGRATION_MP_POINT.md`, `docs/THERMAL_TICKET_SPEC.md`, `docs/PERIPHERALS.md`, `docs/ACCEPTANCE_TESTS.md`, `docs/UI_SPEC.md`. El prototipo de referencia está en `docs/mockup/` y las guías de arranque del operador en `docs/handoff/`.

---

## 1. Qué construir

Un **POS de bazar** web que corre **localmente** en una caja: FastAPI + Uvicorn, Jinja2 + HTMX (no SPA), PostgreSQL (docker-compose), SQLAlchemy + Alembic, `python-escpos`, integración Mercado Pago **API de Orders** para Point. Alcance v1 en `docs/PRD.md`. **No** rediseñar: las decisiones están congeladas; ante disonancia real, **detente y pregunta**.

---

## 2. Estructura del repo

Sigue la estructura de `docs/ARCHITECTURE.md` §4.

```
maschinario-bazar/
├── CLAUDE.md               # este archivo (en la raíz; Claude Code lo lee solo)
├── README.md               # guía operativa de la caja
├── docker-compose.yml      # servicio db (postgres) + volumen
├── .env.example            # plantilla de variables (sin secretos reales)
├── .gitignore              # ignora .env, caches, .venv, respaldos, etc.
├── requirements.txt
├── alembic.ini
├── alembic/                # migraciones
├── app/                    # main, config, db, deps, seed, models, routers, services, templates, static
├── docs/                   # diseño aprobado (PRD, ARCHITECTURE, …) + mockup/ + handoff/
└── tests/                  # pytest
```

**Visibilidad:** el residuo planeó repo **privado**; por decisión del titular (04-jun-2026) el repo se publicó **público** con un **ruleset de protección en `main`**, para habilitar protección de rama en el plan Free de GitHub. No hay secretos versionados (`.env` está en `.gitignore`).
...
> Nota local: si el host ya ocupa el 5432, publica el contenedor en otro puerto con `DB_HOST_PORT` y apunta `DATABASE_URL` a ese puerto. La cookie de sesión usa `APP_SECRET_KEY` (define uno propio en producción).
...
La DB es **precondición**: la app no inicia operaciones de venta sin `db` arriba (ver `docs/ARCHITECTURE.md` §6).
...
- Framework: **pytest**. Implementar los criterios de `docs/ACCEPTANCE_TESTS.md`.
...
- Plantillas Jinja2 con **parciales** para los swaps HTMX (ver `docs/UI_SPEC.md` §7).
...
- Repo **público** con `main` protegida por **ruleset** (ver §2).
- Trabajo en ramas `feat/*`, `fix/*`; **PR a `main`** (push directo bloqueado salvo bypass de admin).
...
- Entregar `README.md` (raíz) con los pasos de §4–§6 para que el titular opere la caja.
...
- **CFDI:** la app **no timbra**; solo marca y exporta a RFC genérico (`docs/ARCHITECTURE.md` ADR-004). Validación fiscal es responsabilidad del titular/contador.

---

*CLAUDE.md · PRY-F-0001.1.8 Maschinario – Bazar · v1.0 · 01-jun-2026 · Residuo para Claude Code (reubicado a la raíz el 04-jun-2026).*
